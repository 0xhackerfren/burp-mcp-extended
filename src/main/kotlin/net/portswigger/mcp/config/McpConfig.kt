...
class McpConfig(storage: PersistedObject, private val logging: Logging) {
    ...
    var requireScannerApproval by storage.boolean(true)

    private var _alwaysAllowScanner by storage.boolean(false)
    var alwaysAllowScanner: Boolean
        get() = _alwaysAllowScanner
        set(value) {
            if (_alwaysAllowScanner != value) {
                _alwaysAllowScanner = value
            }
        }
