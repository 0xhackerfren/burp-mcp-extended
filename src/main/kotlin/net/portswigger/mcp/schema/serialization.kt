...
// Site map entry types (metadata only, no request/response bodies)
...
fun burp.api.montoya.http.message.HttpRequestResponse.toSiteMapEntry(): SiteMapEntry {
    ...
    return SiteMapEntry(
        ...
        mimeType = resp?.statedMimeType()?.name,
        contentType = req?.contentType()?.name,
        params = req?.parameters()?.map { p ->
            ParamInfo(
                name = p.name(),
                type = p.type().name,
                value = p.value().take(50)
            )
        } ?: emptyList()
    )
}

fun AuditIssue.toIssueSummary(): IssueSummary {
    return IssueSummary(
        name = name(),
        severity = severity().name,
        confidence = confidence().name,
        baseUrl = baseUrl(),
        issueType = definition().name(),
        typeIndex = definition().typeIndex()
    )
}
